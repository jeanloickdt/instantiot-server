...
object ServerConfig {
    ...
    /** Une seconde : sous ce seuil, le remede devient la maladie. */
    const val FLUSH_PERIOD_MIN_MS: Long = 1_000L

    /**
     * Une minute. La derivation relit un palier entier pour en recalculer un
     * autre ; la lancer plus souvent que le seau qu'elle agrege n'apporte rien.
     */
    const val ROLLUP_PERIOD_MIN_MS: Long = 60_000L
    ...
    /**
     * Une seconde. Sous ce seuil, le vidage passe plus de temps a prendre et
     * rendre le verrou d'ecriture qu'a ecrire — le remede devient la maladie.
     */
    /**
     * La cadence de la minuterie de vidage, en millisecondes.
     *
     * - **Plus court** — moins de perte a un arret brutal, mais plus de
     *   transactions, chacune prenant le verrou que le tampon evite.
     * - **Plus long** — moins de transactions, mais les tampons RAM gonflent
     *   entre deux tours, et c'est la que la contre-pression commence a
     *   refuser.
     *
     * La boucle est `delay(periode)` PUIS le travail : la periode effective
     * vaut `periode + duree du vidage`. Un vidage lent n'arrive donc pas
     * seulement en retard, il ETIRE l'intervalle.
     */
    var historyFlushPeriodMs: Long = 5_000L
        private set

    /**
     * La cadence de derivation heure et jour, en millisecondes.
     *
     * Separee du vidage expres : la minute doit atteindre le disque vite, la
     * derivation peut attendre. Les deux dans la meme boucle feraient payer a
     * chaque tour de cinq secondes le cout d'un rollup complet.
     */
    var historyRollupPeriodMs: Long = 5 * 60_000L
        private set
    ...
    fun load() {
        ...
        try {
            ...
            historyFlushPeriodMs = props.getProperty("history.flush.period.ms", "5000")
                .toLongOrNull()?.coerceAtLeast(FLUSH_PERIOD_MIN_MS) ?: 5_000L
            historyRollupPeriodMs = props.getProperty("history.rollup.period.ms", "300000")
                .toLongOrNull()?.coerceAtLeast(ROLLUP_PERIOD_MIN_MS) ?: 300_000L
